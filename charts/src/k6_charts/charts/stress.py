...
class StressDegradationChart(BaseChart):
    def render(
        ...
        axes[-1].set_xlabel("Usuarios virtuales (VUs)", fontsize=8)
    ...
    def _panel_latency_vs_vu(
        ...
        ax.plot(
            vu, vu_corr["p95"], color=self.p.s1, lw=self.theme.line_width,
        ...
        ax.set_title("Latencia p95 vs VUs", loc="center", fontsize=9.5, pad=6)
        ax.set_ylabel("ms")
    ...
    def _panel_rps_vs_vu(
        ...
        ax.plot(
            vu, vu_corr["rps"], color=self.p.s2, lw=self.theme.line_width,
            marker="s", ms=self.theme.marker_size, mec="white", mew=1.0, zorder=3,
        )
        ax.fill_between(vu, vu_corr["rps"], alpha=0.10, color=self.p.s2, zorder=1)
        ax.set_title("Throughput (RPS) vs VUs", loc="center", fontsize=9.5, pad=6)
        ax.set_ylabel("req/s")
        ax.set_ylim(bottom=0)
        self.style_ax(ax)
    ...
    def _panel_error_vs_vu(
        ...
        ax.plot(
            vu, vu_corr["err"], color=ecol, lw=self.theme.line_width,
            marker="D", ms=self.theme.marker_size, mec="white", mew=1.0, zorder=3,
        )
        if has_err:
            ax.fill_between(vu, vu_corr["err"], alpha=0.12, color=ecol, zorder=1)
        ax.set_title("Tasa de error vs VUs", loc="center", fontsize=9.5, pad=6)
        ax.set_ylabel("%")
