...
describe("updateLead", () => {
  ...
  it("fetches before state via findUnique with deletedAt:null", async () => {
    (prismadb.crm_Leads.findUnique as ReturnType<typeof vi.fn>).mockResolvedValue({ id: "l1", lastName: "Doe" });
  ...
  it("updates lead with correct fields", async () => {
    (prismadb.crm_Leads.findUnique as ReturnType<typeof vi.fn>).mockResolvedValue({ id: "l1" });
    (prismadb.crm_Leads.update as ReturnType<typeof vi.fn>).mockResolvedValue({
      id: "l1",
      lastName: "Smith",
    });
  ...
  it("sets assigned_to to userId when assigned_to not provided", async () => {
    ...
    await updateLead({ id: "l1", lastName: "Smith" });
    const call = (prismadb.crm_Leads.update as ReturnType<typeof vi.fn>).mock.calls[0][0];
  ...
  it("calls diffObjects with before and after state", async () => {
    ...
    (prismadb.crm_Leads.findUnique as ReturnType<typeof vi.fn>).mockResolvedValue(before);
    (prismadb.crm_Leads.update as ReturnType<typeof vi.fn>).mockResolvedValue(after);
    await updateLead({ id: "l1", lastName: "Smith" });
  ...
  it("returns error on prisma failure", async () => {
    (prismadb.crm_Leads.update as ReturnType<typeof vi.fn>).mockRejectedValue(new Error("DB error"));
    const res = await updateLead({ id: "l1", lastName: "Doe" });
