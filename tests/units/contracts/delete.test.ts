...
describe("deleteContract", () => {
  ...
  it("soft deletes contract by setting deletedAt and deletedBy", async () => {
    (prismadb.crm_Contracts.update as ReturnType<typeof vi.fn>).mockResolvedValue({ id: "c1" });
  ...
  it("returns error on prisma failure", async () => {
    ...
    const res = await deleteContract({ id: "c1" });
    expect(res).toEqual({
      error: "Something went wrong while trying to run DeleteContract action. Please try again.",
    });
  });
});
